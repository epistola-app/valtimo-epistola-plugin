...
The `epistola-document`, `epistola-document-preview`, and `epistola-retry-form` Formio components rely on knowing the active `taskInstanceId` so they can construct correctly-bound calls. Because Angular Elements bootstrap their own injector tree, the components cannot reach Valtimo's `TaskDetailContentComponent` via DI, and no Valtimo service exposes the task id to a custom Formio component at runtime.

**Primary source — server-side form prefill.** The plugin registers a value resolver (`EpistolaTaskValueResolverFactory`, prefix `epistola-task:`) that exposes the current task's id during form prefill — Valtimo's `PrefillFormService` passes the `OperatonTask` itself as the resolver's `VariableScope`. A form field with `properties.sourceKey: "epistola-task:id"` is therefore filled with the task id server-side, and the components read it back from the prefilled form (`readPrefilledTaskId`). This works in **every** task-open flow — both the direct task-open call and the task-list/case-detail flow that bulk-fetches process links and never issues the per-task call.

**Secondary source — HTTP interceptor.** As a zero-config fallback for the direct task-open flow, the plugin module also registers a global HTTP interceptor (`EpistolaTaskContextInterceptor`) that watches for Valtimo's per-task call `GET /api/v2/process-link/task/{taskId}` and publishes the captured `taskId` on a root-provided `EpistolaTaskContextService`. The components prefer the prefilled value and fall back to this service.

Either way the backend authorization is identical and **exact per-task**: the request carries the task id and the controller checks `OperatonTask:VIEW` plus the same-process-instance / same-case binding, so a forged task id resolves to a task the caller cannot view.
