...
The task id is delivered through **server-side form prefill**. The plugin registers a value resolver (`EpistolaTaskValueResolverFactory`, prefix `epistola-task:`) that exposes the current task's id during form prefill — Valtimo's `PrefillFormService` passes the `OperatonTask` itself as the resolver's `VariableScope`. Each component embeds a hidden carrier field with `properties.sourceKey: "epistola-task:id"`, which is therefore filled with the task id server-side; the components read it back from the prefilled form (`readPrefilledTaskId`). This works in **every** task-open flow — both the direct task-open call and the task-list/case-detail flow that bulk-fetches process links and never issues the per-task call. (An earlier approach sniffed the per-task `GET /api/v2/process-link/task/{taskId}` call via an HTTP interceptor; it only worked in the direct-open flow and has been removed in favour of prefill.)

The backend authorization is **exact per-task**: the request carries the task id and the controller checks `OperatonTask:VIEW` plus the same-process-instance / same-case binding, so a forged task id resolves to a task the caller cannot view.
