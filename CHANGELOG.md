...
- **Generated documents can now be archived durably on the case Documents tab via OpenZaak (ZGW), wired on the `subsidy` case — no plugin code changes.** (Verified live: OpenZaak provisioning + API, all four ZGW plugin connections deploy, and the subsidy process's `create-zaak` creates a real zaak in OpenZaak through the full Valtimo→Catalogi API→auth path. and the **full chain was run end-to-end against the mock backend**: a Valtimo-started subsidy case generated 3 real Epistola PDFs, downloaded them, stored them in the Documenten API and linked all 3 to the zaak (verified via the mock API). Three fixes were needed to get there: `taal`/`status` are non-null `store-temp-document` properties (added to the process-link), and the mock had to emit offset-free timestamps (`registratiedatum` deserializes as java `LocalDateTime`) and honor the `informatieobject` query filter (else the link-exists check skips documents 2 and 3).) OSS Valtimo ships no durable non-ZGW document store (its `LocalResourceService` is a stub that throws `NotImplementedError`; `TemporaryResourceStorageService` is non-durable scratch storage), so the case Documents tab is served through the ZGW Documenten/Zaken APIs. The Epistola plugin reaches them purely by composition: `epistola-download-document` (→ `TEMPORARY_RESOURCE`) → `documentenapi:store-temp-document` → `zakenapi:link-document-to-zaak`, with a `zakenapi:create-zaak` at process start so the documents have a zaak to attach to.
  - **Local ZGW backend — lightweight mock by default, real OpenZaak opt-in.** Plain `docker compose up` starts `ved-zgw-mock` (`docker/zgw-fake/server.py`): a tiny, dependency-free, stateful in-memory fake of the ZGW Documenten/Zaken/Catalogi APIs on `:8002` — no PostGIS/Redis/Celery/emulation, native on arm64/amd64. It implements the create→store→link→list→download surface the Valtimo plugins use, so the subsidy demo runs and the Documents tab shows the PDFs without a real ZGW backend. For full-fidelity testing, `docker compose --profile openzaak up -d` (after `docker compose stop ved-zgw-mock`) brings up a self-contained real OpenZaak — its own PostGIS DB + Redis + web + Celery worker, notifications disabled; amd64-only (emulated on arm64). Provisioning is automatic via `ved-openzaak-init`: a `valtimo` API client (≥32-char secret) with full authorizations and a published catalogus + zaaktype + informatieobjecttype. See `docker/zgw-fake/README.md` and `docker/openzaak/README.md`.
  - **Backend dependency:** added `com.ritense.valtimo:openzaak-plugin-authentication` (the `openzaak` auth plugin providing `DocumentenApiAuthentication`/`ZakenApiAuthentication`) to the test-app; `documenten-api`/`zaken-api`/`catalogi-api` were already present.
  - **Plugin connections** (`test-app/.../config/zgw.pluginconfig.json`, auto-deployed): `openzaak` (client credentials), `documentenapi`, `zakenapi`, `catalogiapi`, with URLs/secrets overridable per environment via `ZGW_*` env vars (defaults target the local ZGW backend — mock or `openzaak` profile — at `http://localhost:8002`). Note: the plugin-config placeholder resolver does not support `${VAR:default}`, so the `ZGW_*` defaults live in `application.yml` and the JSON references the bare property keys.
  - **Subsidy process** (`subsidy-decision-package`): added a `create-zaak` start task and wired the previously-empty `store-doc{1,2,3}` / `link-doc{1,2,3}` placeholder tasks to the Documenten/Zaken API actions (`store-temp-document` with `taal`/`status`, then `link-document-to-zaak`); the three downloads remain `TEMPORARY_RESOURCE`.
  - **UI display via a ZGW documents tab component.** OSS Valtimo 13.32 lists a case's ZGW documents from `GET /api/v2/zaken-api/document/{documentId}/files` (resolves the case→zaak link, returns the zaak's `zaakinformatieobjecten` as `DocumentenApiDocumentDto`s) — exactly what the `store-temp-document` + `link-document-to-zaak` flow produces — but it ships **no** component for the `DOCUMENTEN_API` documents tab (`ZGW_DOCUMENTEN_API_DOCUMENTS_COMPONENT_TOKEN` is an unfilled extension point; the standard `relatedFiles` tab cannot show ZGW documents and the persisted related-file model can't carry the `pluginConfigurationId` needed for documenten-api download). The test-app fills that slot with a small standalone `ZgwDocumentsTabComponent` (`test-app/frontend/src/app/zgw-documents/`) bound to the token in `AppModule`, with `uploadProvider: UploadProvider.DOCUMENTEN_API`. It lists the case's documents and downloads each via `GET /api/v1/documenten-api/{pluginConfigurationId}/files/{fileId}/download` — no `ResourceService`, no backend code. Caveat: `uploadProvider` is global, so cases that do **not** create a zaak (example/permit/objection/bulk-letters) show an empty/erroring documents tab until they too get a `create-zaak` step.
