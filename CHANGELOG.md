...
- **Guided expression editing is available for free-form generate-document expressions.** Filename, correlation ID, variant attributes, and each simple data-mapping field share the visual JSONata experience and process-context suggestions. Variant and environment expressions instead use their resource dropdown when an exact option matches and a validated raw expression textarea otherwise. Static mapping objects retain complex leaf expressions without rewriting them, while structurally dynamic mappings remain in the Monaco-based whole-mapping Advanced editor.
...
- **Variant and environment expressions now use the same inline mode layout as other expression fields.** Exact string literals are presented through an attached select box, dynamic or unmatched values open directly as JSONata, and the old separate `fx` state and duplicated select/expression values have been removed.
- **Variant and environment expression controls now expose their labels and validation context to assistive technology.** Dropdown mode uses the standard Valtimo select label contract, while Advanced mode associates its textarea and mode action with the relevant label, syntax error, or dropdown-unavailable explanation.
- **The ESM test application's Karma suite now exercises rendered expression-select behavior in CI.** Its CommonJS Karma configuration uses a `.cjs` extension, the obsolete webpack `require.context` bootstrap is removed, and headless Chrome verifies Angular template binding, selection persistence, mode switching, and accessible error relationships instead of relying only on class-level tests.
