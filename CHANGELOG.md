...
- **Expression field labels now use consistent Valtimo typography in every editor view.** Visual and Advanced use the same standard input-label component already rendered by Select, keeping filename, correlation ID, variant, environment, and mapping labels visually aligned while preserving accessible naming.
