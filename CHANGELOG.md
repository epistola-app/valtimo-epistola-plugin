...
- **`resultProcessVariable` shape: String → `Map<String, Object>`** (see Added entry above). Migration: BPMNs that read this variable directly (e.g. `${epistolaRequestId}`) must switch to `${epistolaRequestId.requestId}` (or rename the field via the gateway condition). The `check-job-status` action's `requestIdVariable` reader now accepts both shapes transparently for backward compatibility — no changes needed there. In-flight processes that submitted on the previous version still hold the legacy String value and continue to work via the type-tolerant reader; new submissions use the rich object.
