...
- **Structured process variables now expose their known child paths in expression suggestions.** Historic map values are inspected safely so rich results such as `epistolaResult` offer paths including `requestId`, `status`, `documentId`, `errorMessage`, and `jobPath`, while unreadable historic values do not prevent the root variable from being suggested.
