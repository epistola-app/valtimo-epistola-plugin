...
- **Plugin version & changelog on the admin page** — a new global "Changelog" tab on the admin overview shows the running plugin version (`/versions`, also still the header badge) and renders the plugin's `CHANGELOG.md`. The CHANGELOG is copied into the plugin jar at build time (`processResources` → classpath `epistola/CHANGELOG.md`) and served by a new PBAC-gated `GET /api/v1/plugin/epistola/admin/changelog` (`text/markdown`); the service returns a friendly placeholder rather than failing if it is absent. The tab lazy-loads on first open and renders the markdown as a styled, scrollable preformatted block (no markdown-renderer dependency added to the published frontend library, by design).
