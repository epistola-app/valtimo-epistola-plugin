...
- **New example processes in the test-app `example` case** demonstrate more correlation topologies. All reuse the `single-document` generate step + start form and are startable from the UI:
  - `single-document-event-gateway` — an **event-based gateway** races the `EpistolaDocumentGenerated` completion message against a `PT30M` timer catch event; the timer branch throws `EpistolaGenerationTimeout`. Auto-wiring (and a catch-event `epistolaWaitFor` mapping) does **not** work behind an event-based gateway — the execution waits at the gateway, so the catch event's listener/mapping only runs after the message is already delivered, too late to correlate. The token is therefore pinned **upstream on the `generate-document` output** (`camunda:outputParameter epistolaWaitFor = ${epistolaResult.jobPath}`).
  - `single-document-receive-task` — waits with a **receive task** instead of a round intermediate catch event, with an interrupting `PT30M` boundary timer (→ `EpistolaGenerationTimeout`). Receive tasks are **not** auto-wired (the parse listener only attaches to round message catch events), so the `epistolaWaitFor` token is pinned explicitly via a `camunda:inputParameter`.
