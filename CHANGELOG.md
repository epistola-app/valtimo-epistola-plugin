...
- **Declared operations in `compatibility.json`.** The declaration now also lists every Epistola API call the plugin makes (`operations`, by the contract's `operationId`), enabling operation-level compatibility verdicts: a contract release that breaks only calls the plugin never makes no longer marks the plugin incompatible. The list lives in `build.gradle.kts` and is verified against the source by `verifyCompatibilityDeclaration` — generated-client calls are scanned both ways (an undeclared call fails the build, so does a declared-but-unused one); the hand-built HTTP calls are presence-checked by source markers. Local `generateCompatibilityDeclaration` runs now keep the release-stamped `version` field instead of overwriting it with `unspecified`.
- **Machine-readable compatibility declaration (`compatibility.json`).** The plugin now publishes a small machine-readable feed of the `epistola-contract` version it targets (`targetContractVersion`, derived from the `epistola-client` dependency — never hand-authored), for the cross-repo version-compatibility matrix anchored on `epistola-contract`. Generated by `./gradlew generateCompatibilityDeclaration` and guarded in CI by `verifyCompatibilityDeclaration`, which fails the build if the committed file drifts from the derived values. Complements the human-facing [COMPATIBILITY.md](COMPATIBILITY.md).
