...
- **Helm: auto-generate secrets**: All secret values (`keycloakClientSecret`, `pluginEncryptionSecret`, `operatonAdminPassword`, `keycloakAdminPassword`, `epistolaClientSecret`) are now auto-generated with random values when left empty, removing hardcoded defaults from `values.yaml`. Generated secrets are persisted across Helm upgrades by looking up the existing Secret. Explicit values and `secrets.existingSecret` continue to work as before.
