...
- **The expression picker now accepts any valid JSONata variable missing from context suggestions.** Unmatched names preserve the root the author enters, such as `paymentReference` → `$paymentReference`, `external.payload` → `$external.payload`, and `pv.paymentReference` → `$pv.paymentReference`.
