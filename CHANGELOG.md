...
- **The `download-document` action no longer breaks document generation for real-sized documents.** It previously stored the downloaded PDF as a Base64 **string** process variable. Operaton keeps string variables in a `varchar(4000)` column (`ACT_RU_VARIABLE`/`ACT_HI_VARINST.TEXT_`), so any document whose Base64 exceeded 4000 chars failed the historic-variable INSERT with `value too long for type character varying(4000)`. Because the download service task ran in the same transaction as the message correlation that resumed the process, the failure rolled the correlation back and the `EpistolaDocumentGenerated` intermediate catch event was never completed — the process stalled at the wait state. The action now stores the **raw PDF bytes as an Operaton `bytes` variable** (byte-array backed, no size limit). A `bytes` variable rather than a `FileValue` is used deliberately: Valtimo serializes every process variable to JSON when a user task is opened, and a `FileValue` exposes its value as a `ByteArrayInputStream` that Jackson cannot serialize (`No serializer found for ByteArrayInputStream`), whereas a `byte[]` serializes cleanly as a Base64 string — preserving the original Base64 contract for any consumer. (`EpistolaPlugin.downloadDocument`)

  > **Interim.** Because Valtimo serializes the full variable map into the task-detail response, this `byte[]` still ships the whole (private) document to the browser on every task open, and is not a durable home for long-lived processes. The long-term direction — making the storage target a configurable strategy with an `epistola-on-demand` default — is tracked in [ADR 0001](docs/adr/0001-download-document-content-storage.md).
