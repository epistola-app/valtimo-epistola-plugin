...
- **Local frontend dev auth can now be switched without copying config files.** `test-app/frontend/src/assets/config.js` defaults to Keycloak but accepts `?authProvider=authentik` or `?authProvider=keycloak`, persisted in `localStorage`, for source-mode testing. Authentik mode fills the local Compose OIDC issuer/client/callback defaults.
