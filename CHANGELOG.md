...
- **Docker Compose now includes a local `authentik` profile.** The profile starts Authentik server and worker containers backed by the existing Postgres service, exposes Authentik on `http://localhost:9000`, and mounts a startup blueprint that provisions the Valtimo OAuth2/OIDC application, role claim mapping, and demo users for source-mode testing.
