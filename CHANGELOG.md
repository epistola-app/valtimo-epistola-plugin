...
- **Epistola components dropped in the Formio builder now keep their hidden task-id carrier, so they work when the task is opened.** Previously _every_ form authored through the builder was saved **without** the carrier, and the preview, download, and retry-form components failed closed with _"… is only available from within a user task"_, regardless of how the task was opened. Only hand-authored form JSON (the classpath retry form and the test-app forms) carried it — which is why the test suite and the dev flows stayed green while real usage broke.
  ...
  - **Fix:** each task-bound component is now additionally wrapped in `withPrefilledTaskIdCarrier` (`components/valtimo-formio-adapter.ts`), which re-adds the carrier after Formio's filter. Applied to all three components. The carrier is matched on key **and** source key, because Formio's `defaultsDeep` merges the default `components` array element-wise and would otherwise let an unrelated child masquerade as the carrier.
...
- **Upgraded the Epistola contract client from `1.0.1` to `1.1.0`.** The matching Prism mock-server fixtures now use `1.1.0`; the additive contract release publishes complete catalog wire types and adds wire-v6 discovery, license, list-spacing, and PDF/A metadata without changing the plugin's bundled wire-v4 catalogs or Epistola Suite compatibility floor.
...
- **One guided expression editor now covers free-form and resource-backed generate-document expressions.** Filename, correlation ID, variant/environment selection, variant attributes, and each simple data-mapping field share the same canonical JSONata state, visual editing, and process-context suggestions. Supplying resource options adds a Select view alongside Visual and Advanced, while structurally dynamic mappings remain in the Monaco-based whole-mapping Advanced editor.
- **Expression authoring and action-version semantics are documented together.** The data-mapping guide explains visual literals, reference and typed-value chips, per-field Advanced mode, source preservation, and whole-mapping compatibility, while the versioning guide clarifies that both editor modes persist the existing v1 JSONata format and therefore require no v2 migration.
- **The Epistola Admin Problems overview now identifies outdated generate-document actions.** Supported legacy action configurations are reported with their stored and current versions and tell administrators to open and save the action to persist the existing in-editor upgrade.
...
- **Expression field labels now use consistent Valtimo typography in every editor view and field.** Visual and Advanced use the same standard input-label component already rendered by Select, while filename and correlation ID delegate their titles and tooltips to the shared editor instead of wrapping it in larger hand-styled labels. Filename, correlation ID, variant, environment, and mapping labels therefore remain visually aligned while preserving accessible naming.
...
- **The expression picker now accepts any valid variable missing from context suggestions without guessing its scope.** An unmatched path offers case, process, document, and independent JSONata forms—such as `$case.paymentReference`, `$pv.paymentReference`, `$doc.paymentReference`, and `$paymentReference`—while explicitly scoped input is preserved and known matches are deduplicated.
...
- **Select, Visual, and Advanced expression views now use one lossless inline cycle action.** Its icon and accessible label show the next available view; unavailable views are skipped, and the action is disabled when complex JSONata can only remain in Advanced. Fields without resource options continue to cycle only between Visual and Advanced.
...
- **Structured process variables now expose their known child paths in expression suggestions.** Generate-document process links provide `requestId`, `status`, `documentId`, `errorMessage`, and `jobPath` immediately for every configured result variable, even before a process has run. Historic map values are still inspected safely for arbitrary nested objects, while unreadable historic values do not prevent the root variable from being suggested.
- **Variant and environment expressions now use the shared editor instead of a parallel select/expression implementation.** Exact available strings prefer Select, simple dynamic or unmatched expressions use Visual, and complex JSONata uses Advanced; cycling never rewrites the stored expression. The mode button remains aligned to the 40px input control instead of stretching across the select label.
- **Resource-backed expression controls now expose their labels and validation context to assistive technology.** Select uses the standard Valtimo label contract, while Visual and Advanced associate their editing surface, textarea, and syntax errors with the same field label.
- **The ESM test application's Karma suite now exercises the rendered unified expression editor in CI.** Its CommonJS Karma configuration uses a `.cjs` extension, the obsolete webpack `require.context` bootstrap is removed, and headless Chrome verifies selection persistence, three-view lossless cycling, fixed control sizing, and accessible error relationships instead of relying only on class-level tests.
...
- **The frontend package manifest now leaves its published export map to ng-packagr.** The redundant source `exports` map was removed, while the `module` and `typings` paths remain for local `workspace:*` consumer resolution; the built package still receives generated ESM and type exports without ng-packagr override warnings.
...
- **The project agent guidance now requires a Valtimo changelog impact review for dependency upgrades.** Valtimo backend/frontend bumps must check the release notes for every crossed version and record the expected impact before treating the upgrade as mechanical.
- **The local frontend toolchain now targets current supported releases.** Node.js is pinned to the 24.x LTS line and pnpm is pinned to the current stable 11.x release in both mise and `packageManager`. CI/release workflows now take the pnpm version from `package.json`'s `packageManager` field instead of a hardcoded pin. The pnpm workspace build-script allowlist was migrated to pnpm 11's `allowBuilds` syntax, `.pnpm-store/` is ignored as generated local cache, and the frontend plugin now declares the direct dev dependencies that pnpm 11's stricter layout requires for type-checking.
- **Frontend dependency CVEs surfaced by the lockfile rewrite are remediated where a compatible fix exists.** Workspace `overrides` pin patched transitive versions (including `axios`, `immutable`, `js-yaml`, `lodash`/`lodash-es`, `dompurify`, `jsonata` 2.2.x, and Angular 19.2.25), and `.trivyignore` documents the remaining Angular 19 / `uuid@9` findings that only fix on majors Valtimo does not yet support.
...
- **The transitional task-id carrier repair has been removed.** The backend scan/repair service and admin endpoints, their frontend service models, and the repair controls on the Forms tab are gone. Forms created before the embedded carrier was introduced must be updated at their source by re-dropping the Epistola component.
- **The frontend package now exposes only its supported host integration API.** `EpistolaPluginModule` and `epistolaPluginSpecification` remain public; internal components, services, models, routes, runtime helpers, and manual Form.io registration functions are no longer re-exported from the package entrypoint.
...
- **The npm release job now accepts an already matching package version.** Publishing `1.0.0` no longer stops at `npm version` when the built package manifest is already set to `1.0.0`.
- **The plugin release workflow now ignores Helm chart releases and rejects invalid manual versions.** Only GitHub releases with a `v`-prefixed tag enter the plugin publication jobs, preventing chart releases from starting a doomed Maven/npm publication run.
- **The release workflow now preserves authored GitHub release notes.** Installation instructions live in a marked, replaceable block instead of overwriting the full release body, so reruns stay idempotent and 1.0 release notes are retained.
- **Release builds now enforce the same code gates as main CI.** Frontend lint, formatting, headers, strict type-checking, singleton resolution, unit tests, and the consumer build must pass, together with the backend test-app E2E suite, before any Maven or npm publication job starts.
...
- **The Epistola admin menu registration now updates Valtimo's configured menu before reload.** `EpistolaMenuService` adds `Admin > Epistola` to the host menu configuration and also keeps the append hook for already-built menus, so hosts do not need a custom static menu entry.
- **Workspace package injection now mirrors installed-plugin peer resolution.** Local demo builds no longer run the linked Epistola plugin against separate development instances of Valtimo peer packages, preventing duplicate Angular singletons such as `MenuService` and Form.io services.
- **Bundled Epistola catalog templates now use Suite's synthetic root node shape.** The municipality demo catalog and reusable test fixture now declare the document root node with `type: "root"` instead of rendering it as a `container`, and the demo catalog release is bumped to `1.2` so existing installations redeploy the corrected content. A classpath regression test now fails if bundled template resources drift back to a non-root root component.
...
- **Demo authentication can now target Authentik as a selectable OIDC provider.** Keycloak remains the default, while the test-app frontend can switch to an Authentik-compatible authorization-code + PKCE adapter through its runtime `authProvider` config. The backend adds an `authentik` Spring profile that validates generic OIDC JWTs and maps Keycloak-shaped `realm_access.roles` / `resource_access.*.roles` claims into Valtimo authorities without instantiating the Keycloak Admin client. Docker Compose exposes Authentik OIDC env vars and an alternate frontend config file; the Helm chart adds `auth.provider` and `auth.oidc.*` values for Keycloak-or-Authentik deployments.
- **Docker Compose now includes a local `authentik` profile.** The profile starts Authentik server and worker containers backed by the existing Postgres service and a profile-scoped Redis, exposes Authentik on `http://localhost:9000`, and mounts a startup blueprint that provisions the Valtimo OAuth2/OIDC application, role claim mapping, and demo users for source-mode testing.
- **Local frontend dev auth can now be switched without copying config files.** `pnpm start` remains the Keycloak source-mode frontend, while `pnpm start:authentik` serves an Authentik-specific index/runtime config that fills the local Compose OIDC issuer/client/callback defaults without a URL query parameter. The pre-built container demo keeps Keycloak as the default and can run Authentik mode with `--env-file docker/containers/authentik.env`.
- **The Helm chart now documents and defaults the Keycloak-or-Authentik switch.** Keycloak remains the default provider, while `auth.provider=authentik` disables bundled Keycloak resources, enables the backend `authentik` Spring profile, renders Authentik OIDC frontend/backend settings, and requests the `roles` scope needed for Authentik role claims.
...
- **The test-app frontend now keeps Angular auth modules statically importable.** Keycloak and Authentik modules are both imported by the root app module, while the runtime `authProvider` switch only selects the active auth config/providers. This preserves the local `pnpm start` workflow and the separate `pnpm start:authentik` workflow.
- **Authentik mode now satisfies Valtimo frontend components that still inject `KeycloakService`.** The test-app provides a small compatibility service that delegates to Keycloak in Keycloak mode and exposes Authentik roles/profile/token data in Authentik mode, avoiding undefined `resourceAccess` reads during menu construction.
- **The Authentik frontend dev configuration now serves the app at `/`.** The alternate Authentik index file is emitted as `index.html`, so `pnpm start:authentik` works at `http://localhost:4200` without a query parameter or alternate path.
...
- **Catalog-import failures now carry the real cause with a correct status class and operator-actionable remediation (#71).** Previously `EpistolaServiceImpl.importCatalog` discarded the downstream RFC-9457 problem body and rethrew a generic message, and `EpistolaAdminResource.redeployCatalog` mapped every failure to `502 BAD_GATEWAY` — so an operator saw a bare 502 with no reason. Now: (1) `importCatalog` catches `HttpStatusCodeException` (4xx **and** 5xx), preserves the suite's `detail`/`title` and the structured `type` / `version` / `baselineVersion` extensions on `EpistolaApiException` (`extractErrorMessage` is also RFC-9457-aware now, improving the preview path); (2) the redeploy endpoint maps a downstream **4xx → 422 Unprocessable Entity** (a deterministic data problem the operator must fix by shipping a newer catalog) and reserves **502** for 5xx / connectivity failures; (3) for the `catalog-schema-too-old` case the suite's "re-export from a current source" advice — a dead end for a catalog compiled into the application image — is **translated** into actionable guidance ("…upgrade the valtimo-epistola-plugin (and the host application) to a build whose bundled catalogs target wire schema v{baselineVersion}"). The admin UI already prefers `errorMessage`/`detail`, so the richer reason surfaces inline next to the redeploy button with no frontend change.

### Changed

- **Bumped the Epistola contract client `app.epistola.contract:client-spring3-restclient` from `0.6.0` to `0.8.0`** (matching Epistola Suite ≥ 0.26.0, which depends on `epistola-model` `0.8.0`). The generation/read API surface the plugin uses is unchanged; `0.7.0` migrated error responses to RFC 9457 Problem Details (removing the deprecated `ErrorResponse`/`ValidationErrorResponse`/`FieldError` schemas, which the plugin never referenced) and `0.8.0` added the additive `StencilResource.parameterSchema`. The Prism mock-server image used by the integration test and docker-compose is bumped to the matching `mock-server:0.8.0`.
