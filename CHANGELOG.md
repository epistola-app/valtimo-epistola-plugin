...
- **The document preview, download, and retry-form components now work in every Valtimo task-open flow** — previously they failed with _"… is only available from within a user task"_ when a task was opened via the task-list / case-detail view. Those components need the active `taskInstanceId` to authorize their backend calls (`OperatonTask:VIEW`). They obtained it only from `EpistolaTaskContextInterceptor`, which sniffs the per-task `GET /api/v2/process-link/task/{id}` call — but Valtimo's task-list/case-detail flow bulk-fetches process links (`GET /api/v1/process/{pid}/tasks/process-link`) and renders the form inline, so that call never fires and the task id was never captured. (The direct task-open flow, which the dev test-app uses, _does_ fire it — which is why it worked locally but not at customers.) No task-pinning value (task id, activity id, or execution id) is reachable by a Formio component at runtime in that flow, so the id is now delivered through **server-side form prefill**, which runs in all flows:
  - A plugin `ValueResolverFactory` (`EpistolaTaskValueResolverFactory`, prefix `epistola-task:`) exposes the current task's `id`/`executionId`/`taskDefinitionKey` at prefill time (Valtimo passes the `OperatonTask` itself as the resolver's `VariableScope`).
  - A hidden carrier field with `properties.sourceKey: "epistola-task:id"` is prefilled with the task id; the Formio wrappers read it back (`readPrefilledTaskId`) and forward it to the Angular component, which prefers it over the interceptor and only fails closed when both are absent. The interceptor is kept as a secondary, zero-config source for the direct-open flow.
  - Authorization is unchanged and stays **exact per-task** (`requireTaskBoundTo`); a forged task id resolves to a task the caller has no `VIEW` on. Forms placing the preview/download component must include the carrier field (baked into the bundled retry form; documented for author forms in [docs/document-component.md](docs/document-component.md)). Covered by `EpistolaTaskValueResolverFactoryTest` and `prefilled-task-id.spec.ts`.
